...
DEFAULT_QUALITY = 90
assert DEFAULT_QUALITY >= 0 and DEFAULT_QUALITY <= 100 #  PIL save image quality range is [0, 100]
assert DEFAULT_QUALITY % 5 == 0 # make adaptive compression easier, rather than search all 100 possibilities
SAVED_IMAGES = [] #  should only have one saved image and deleted failed attempts with adpative compression 
...
def main() -> None:
    ...
    def save_image(image_quality=DEFAULT_QUALITY) -> tuple[str, int]:
        if verbose: print('Attempting quality setting:', CURRENT_IMAGE_QUALITY)
    ...
    if adaptive is not None:
        if compressed_size > adaptive:
            for CURRENT_IMAGE_QUALITY in (possible_quality_values := list(range(DEFAULT_QUALITY - 5, -1, -5))): #  already tried for first quality setting
                save_path, compressed_size = save_image(image_quality = CURRENT_IMAGE_QUALITY)
                SAVED_IMAGES.append(save_path)
                if compressed_size < adaptive:
                    break
